...
check(occurrences(html, '<script src="/gymlog-reliability.js?v=20260916-storage-recovery"></script>') === 1, 'El módulo reliability debe cargarse una vez y saltarse caches antiguas.');
...
check(html.includes('function leaveCompletedWorkout()'), 'El entreno activo solo debe limpiarse despues de guardar la sesion.');
check(html.includes('const previous = structuredClone(state.workoutLog[idx]);'), 'El editor debe poder restaurar la sesion original si falla el guardado.');
check(reliability.includes('storageKey:STORAGE_KEY'), 'La cola de nube debe referenciar el estado local sin duplicar todo el historial.');
check(reliability.includes('function compactLegacyPendingState()'), 'Las colas antiguas deben compactarse usando el estado durable mas reciente.');
check(reliability.includes('const storedStateRaw = localStorage.getItem(STORAGE_KEY);'), 'La compactacion solo debe ocurrir cuando existe un estado principal durable.');
check(reliability.includes('function removeOldestRecoverySnapshot()'), 'Un guardado bloqueado debe poder liberar copias internas antiguas.');
check(reliability.includes('while(savedLocally === false && removeOldestRecoverySnapshot())'), 'El guardado debe reintentarse al recuperar espacio interno.');
check(reliability.includes('let savedLocally = baseSave.apply(this, arguments);'), 'La cola de nube debe crearse despues del guardado local.');
check(reliability.includes('pendingRecoveredIntoState = true'), 'La recuperacion debe marcar cuando la cola ya esta representada en memoria.');
check(html.includes('const activeWorkoutBackup = localStorage.getItem(ACTIVE_WORKOUT_KEY);'), 'Finalizar debe apartar temporalmente la copia duplicada del entreno activo.');
check(html.includes('localStorage.setItem(ACTIVE_WORKOUT_KEY, activeWorkoutBackup)'), 'Un fallo de guardado debe restaurar la sesion activa exacta.');
check(html.includes("updateViaCache:'none'"), 'La PWA debe buscar siempre la ultima version del service worker.');
check(serviceWorker.includes("gymlog-web-v14-20260916-storage-recovery"), 'La cache de la PWA debe invalidar la version anterior.');
check(reliability.includes('if(result) Promise.resolve(result)'), 'La nube solo debe sincronizar una finalizacion local confirmada.');
check(html.includes('max-width: calc(100vw - 24px)'), 'Los avisos criticos deben caber y envolver texto en movil.');
