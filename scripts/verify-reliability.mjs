...
check(reliability.includes('function compactEquivalentPendingState()'), 'Las colas antiguas redundantes deben poder compactarse sin perder datos unicos.');
check(reliability.includes('const isRedundant = !!storedState && JSON.stringify(pending.state) === storedState'), 'La compactacion debe reconocer copias identicas del estado durable.');
check(reliability.includes('if(!isRedundant && !pendingRecoveredIntoState) return null'), 'Una cola con datos unicos solo puede compactarse despues de recuperarla en memoria.');
check(reliability.includes('const savedLocally = baseSave.apply(this, arguments);'), 'La cola de nube debe crearse despues del guardado local.');
check(reliability.includes('localStorage.setItem(PENDING_STATE_KEY, pendingBackup)'), 'Una compactacion fallida debe restaurar la cola pendiente original.');
